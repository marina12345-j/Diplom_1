...
class TestDB:
    ...
    def test_available_buns_db_success(self, index_bun, bun_name, bun_price):  # проверка метода available_buns получения списка доступных булок из базы (имя и стоимость булки)
        db = Database()
    ...
    def test_available_ingredients_db_success(self, index_i, type_ingredient, name_ingredient, price_ingredient):  # проверка метода available_ingredients получения списка доступных ингредиентов из базы (имя и стоимость булки)
        db = Database()
